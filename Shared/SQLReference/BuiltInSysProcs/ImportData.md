[Open topic with navigation](../../../index.html#Shared/SQLReference/BuiltInSysProcs/ImportData.html)

<a href="" id="BuiltInSysProcs.ImportData"></a>[]()SYSCS\_UTIL.IMPORT\_DATA
===========================================================================

The SYSCS\_UTIL.IMPORT\_DATA system procedure imports data to a subset of columns in a table. You choose the subset of columns by specifying insert columns.

The <span class="CodeFont">SYSCS\_UTIL.SYSCS\_IMPORT\_DATA</span> system procedure, which operates similarly and was available in earlier versions of Splice Machine, has been deprecated.
...
<span class="CodeFont">SYSCS\_UTIL.SYSCS\_IMPORT\_DATA</span> displays a summary of the import process results that looks like this:
...
-   A primary key violation: <span class="CodeFont">[SYSCS\_UTIL.IMPORT\_DATA](#)</span> will only work correctly if the table into which you are inserting/updating has primary keys.
...
<span class="autonumber"><span class="noteAutoNum">RELEASE NOTE:  </span></span>Run a full compaction on tables into which you have imported a large amount of data, using the <span class="CodeFont">[SYSCS\_UTIL.SYSCS\_PERFORM\_MAJOR\_COMPACTION\_ON\_TABLE](PerformMajorCompactionOnTable.html)</span> system procedure.
...
This example shows the <span class="CodeFont">IMPORT\_DATA</span> call used to import the Players table into our documentation examples database:
...
-   [<span class="CodeFont">SYSCS\_UTIL.UPSERT\_DATA\_FROM\_FILE</span>](UpsertDataFromFile.html)
